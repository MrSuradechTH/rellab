...
class _CHARTState extends State<CHART> {
  ...
  void initState() {
    ...
    Timer.periodic(const Duration(milliseconds: updatedelay), (timer) async {
      ...
      if (chartstart == true) {
        // print(xmax);
        print(screensize[0].toString() + " : " + ((intervalvalue[0] - ((screensize[0] / 1500) * intervalvalue[0]).ceil() > 10 ? 10 : (intervalvalue[0] - (screensize[0] / 1500) * intervalvalue[0]).ceil() <= 0 ? 1 : (intervalvalue[0] - (screensize[0] / 1500) * intervalvalue[0]).ceil()) * 60).toDouble().toString() + " | " + screensize[1].toString() + " : " + ((intervalvalue[1] - ((screensize[1] / 700) * intervalvalue[1]).ceil() > 20 ? 20 : (intervalvalue[1] - (screensize[1] / 700) * intervalvalue[1]).ceil() <= 0 ? 1 : (intervalvalue[1] - (screensize[1] / 700) * intervalvalue[1]).ceil()) * 5).toDouble().toString());
        // print(_data.length);
        getdata(mode[0]);
        timenow = datain['time'] ?? "";
        if (timenow != lasttime) {
      ...
      child: Scaffold(
        ...
        body: LayoutBuilder(
          builder: (context, layoutdata) {
            screensize[0] = layoutdata.maxWidth;
            screensize[1] = layoutdata.maxHeight;
            return Container(
                    ...
                    Container(
                      ...
                      child: SfCartesianChart(
                        ...
                        primaryXAxis: CategoryAxis(
                          //String
                          interval: ((intervalvalue[0] - ((screensize[0] / 1500) * intervalvalue[0]).ceil() > 12 ? 12 : (intervalvalue[0] - (screensize[0] / 1500) * intervalvalue[0]).ceil() <= 6 ? 6 : (intervalvalue[0] - (screensize[0] / 1500) * intervalvalue[0]).ceil()) * 5).toDouble(),
                          // interval: 60,
                          labelRotation: 90,
                        ...
                        primaryYAxis: NumericAxis(
                          ...
                          minimum: -80,
                          maximum: 180,
                          interval: ((intervalvalue[1] - ((screensize[1] / 700) * intervalvalue[1]).ceil() > 20 ? 20 : (intervalvalue[1] - (screensize[1] / 700) * intervalvalue[1]).ceil() <= 0 ? 1 : (intervalvalue[1] - (screensize[1] / 700) * intervalvalue[1]).ceil()) * 5).toDouble(),
                          tickPosition: TickPosition.inside,
                          title: AxisTitle(
                            text: "Temperature(°C)",
                            textStyle: const TextStyle(
